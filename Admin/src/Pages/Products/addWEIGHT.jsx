...
const AddWEIGHT = () => {
        ...
        if (editId === undefined || editId === '') {
            postData(`/api/product/productWEIGHT/create`, {
                ...
                if (res.error === false) {
                    ...
                    context.alertBox("error", res?.message || "Failed to create weight");
                    setIsLoading(false);
                }
        ...
        if (editId !== '') {
            ...
            editData(`/api/product/productWEIGHT/${editId}`, {
                ...
                if (res?.data?.error === false) {
                    ...
                    context.alertBox("error", res?.data?.message || "Failed to create weight");
                    setIsLoading(false);
                }
    ...
    const deleteItem = (id) => {
        deleteData(`/api/product/productWEIGHT/${id}`).then((res) => {
            if (res?.data?.error === false) {
                ...
                context.alertBox("error", res?.data?.message || "Failed to delete weight");
            }
        }).catch((err) => {
            context.alertBox("error", "Something went wrong!");
        });
    ...
    const editItem = (id) => {
        fetchDataFromApi(`/api/product/productWEIGHT/${id}`).then((res) => {
            if (res?.error === false) {
                ...
                context.alertBox("error", res?.message || "Failed to fetch weight data");
            }
        }).catch((err) => {
            context.alertBox("error", "Something went wrong!");
        });
    ...
    return (
        <>
            ...
            <div className="ccard my-4 mx-auto pt-5 pb-5 shadow-md sm:rounded-lg bg-white w-full sm:w-full md:w-[80%] lg:w-[65%]">
                <form className='form py-3 p-6' onSubmit={handleSubmit}>
                    <div className="col mb-4">
                        <h3 className='text-[14px] font-[500] mb-1 text-black'>{editMode ? "Edit" : "Add"} Product Weight</h3>
                        <input type='text' className='w-full h-[40px] border border-[rgba(0,0,0,0.2)] focus:outline-none focus:border-[rgba(0,0,0,0.4)] rounded-smp-3 text-sm'
                            name='weight'
                            value={weight}
                            placeholder={editMode ? 'Edit product weight' : 'Enter product weight'}
