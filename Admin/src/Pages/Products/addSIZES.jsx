        ...
        if (editId !== '') {
            ...
            editData(`/api/product/productSize/${editId}`, {
                ...
                if (res?.data?.error === false) {
                    ...
                    context.alertBox("error", res?.data?.message || "Failed to create size");
                    setIsLoading(false);
                }
    ...
    const deleteItem = (id) => {
        deleteData(`/api/product/productSize/${id}`).then((res) => {
            if (res?.data?.error === false) {
                ...
                context.alertBox("error", res?.data?.message || "Failed to delete size");
            }
        }).catch((err) => {
            context.alertBox("error", "Something went wrong!");
        });
    ...
    const editItem = (id) => {
        fetchDataFromApi(`/api/product/productSize/${id}`).then((res) => {
            if (res?.error === false) {
                ...
                context.alertBox("error", res?.message || "Failed to fetch size data");
            }
        }).catch((err) => {
            context.alertBox("error", "Something went wrong!");
        });
                ...
                <form className='form py-3 p-6' onSubmit={handleSubmit}>
                    <div className="col mb-4">
                        ...
                        <input type='text' className='w-full h-[40px] border border-[rgba(0,0,0,0.2)] focus:outline-none focus:border-[rgba(0,0,0,0.4)] rounded-smp-3 text-sm'
                            name='size'
                            value={size}
                            placeholder={editMode ? 'Edit product size' : 'Enter product size'}
                                        ...
                                        return (
                                            <tr className='odd:bg-white odd:dark:bg-white even:bg-gray-50 even:dark:bg-gray-100 border-b dark:border-gray-700' key={index}>
                                                ...
                                                <td className='px-6 py-2'>
                                                    <span className='font-[600]'>{item?.size}</span>
                                                </td>
